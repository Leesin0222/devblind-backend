...
import com.yongjincompany.devblind.config.SmsProperties;
...
public class NaverCloudSmsSender implements SmsSender {
    ...
    public void send(String phone, String content) {
        ...
        String uri = "/sms/v2/services/" + smsProperties.ncp().serviceId() + "/messages";
        ...
        smsWebClient.post()
                ...
                .header("x-ncp-iam-access-key", smsProperties.ncp().accessKey())
    ...
    private Map<String, Object> createRequestBody(String phone, String content) {
        ...
        body.put("type", "SMS");
        body.put("from", smsProperties.ncp().senderPhone());
    ...
    private String makeSignature(String method, String url, long timestamp) {
        try {
            String message = String.join("\n", 
                ...
                smsProperties.ncp().accessKey()
            );

            log.debug("시그니처 생성 - 메시지: {}", message);

            SecretKeySpec signingKey = new SecretKeySpec(
                smsProperties.ncp().secretKey().getBytes(StandardCharsets.UTF_8), 
                "HmacSHA256"
            );
